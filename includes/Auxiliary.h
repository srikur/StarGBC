...
struct Serial {
    ...
    void WriteSerial(const uint16_t address, const uint8_t value, const bool gbc) {
        switch (address) {
            case 0xFF01:
                data_ = value;
                break;
            case 0xFF02:
                control_ = (control_ & 0x7C) | (value & 0x83);
                if (control_ & 0x80) {
                    StartTransfer(gbc);
                }
                break;
    ...
    void StartTransfer(const bool doubleSpeed) {
        if ((control_ & 0x01) == 0) {
            active_ = false;
            return;
        }
